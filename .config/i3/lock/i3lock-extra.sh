#!/usr/bin/env bash
## Taken from https://git.fleshless.org/misc/tree/i3lock-extra ##
...
usage() {
    while read; do printf '%s\n' "$REPLY"; done <<- EOF
		  ...
		  --umask|-u                      Set umask.
	  	  --tmpdir|-d                     Directory to store temporary 
		  				    files in.  This should not
                                                    be accessible by other users!
	          --image_file|-i                 Lock screen with an image.
		  --screenshot|-s                 Lock screen with a screenshot 
                                                    of your current workspace.

		   # Modifiers (apply to both screenshots and specified images):
		   --overlay-image|-o              Overlay another image over the
                                                     main one.  Gets applied 
						     after all other modifiers.
		   --grayscale|-g                  Make the image grayscale.
	           --pixelize|-p                   Pixelize the image.
	      	   --pixelize-scale|-P             Sets the number by which the 
		     				     image is resized down and up
		                                     again to achieve the 
		    				     pixelize effect. For example,
		   				     4 means that it will be 
		    				     resized to 1/4 of the original 
		   				     and blown back up.
...
image_prepare() {
	...
	do
            ...
            time convert "$image_file" "$overlay_image" -geometry +$PX+$PY -composite -matte "$cfg_tmpdir/lockbg.png"
            echo "done"
	done
...
main() {
    ...
    if (( f_secloop )); then
	until i3lock -B=1 -n -t -i "$image_file" --radius=102; do
	    true
	done
    else
	i3lock --radius=300 -n -t -i "$image_file"
    fi
}
